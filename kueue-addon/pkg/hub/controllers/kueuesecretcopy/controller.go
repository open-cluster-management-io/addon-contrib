...
func (c *kueueSecretCopyController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	// Check if cluster permissions are ready for operations
	ready, err := c.isClusterPermissionReady(clusterName)
	if err != nil {
		return err
	}
	if !ready {
		logger.Info("Cluster Permission is not ready", "cluster", clusterName)
		return nil
	}
...
// isClusterPermissionReady checks if ClusterPermission AppliedRBACManifestWork condition is true
func (c *kueueSecretCopyController) isClusterPermissionReady(clusterName string) (bool, error) {
	cp, err := c.permissionLister.ClusterPermissions(clusterName).Get(common.MultiKueueResourceName)
	if err != nil {
		return false, err
	}

	for _, condition := range cp.Status.Conditions {
		if condition.Type == permissionv1alpha1.ConditionTypeAppliedRBACManifestWork && condition.Status == "True" {
			return true, nil
		}
	}
