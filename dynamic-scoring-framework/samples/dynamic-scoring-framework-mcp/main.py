...
class ResetParams(BaseModel):
    ...
    namespace: str = Field(..., description="Target namespace to delete resources from")


class ResetResponse(BaseModel):
    """Response containing reset results"""

    deleted: List[str] = Field(
        default_factory=list, description="List of deleted resource names"
    )
    failed: List[str] = Field(
        default_factory=list, description="List of resources that failed to delete"
    )
...
def perform_optimization(params: OptimizationParams) -> OptimizationResponse:
    ...
    print(f"Resource Capacity Keys: {list(resource_capacity.keys())}")
    print(f"Preference Scores Keys: {list(preference_scores.keys())}")
        ...
        for d in devices:
            model += (
                (
                    ...
                    <= resource_capacity.get((c, d), 0) * y[(c, d)]
                ),
                f"Capacity_{c}_{d}",
            )
...
GENERATED_LABEL = "dynamic-scoring-framework-mcp/generated"
GENERATED_LABEL_SELECTOR = f"{GENERATED_LABEL}=true"


@app.post("/reset", response_model=ResetResponse)
async def reset(params: ResetParams) -> ResetResponse:
    """Delete all resources generated by the MCP optimization endpoint

    Deletes Placement and PlacementBinding resources in the specified namespace
    that have the label 'dynamic-scoring-framework-mcp/generated: true'.
    """
    namespace = params.namespace
    api = client.CustomObjectsApi()
    deleted = []
    ...
    for resource_type in resource_types:
        try:
            result = api.list_namespaced_custom_object(
                ...
                label_selector=GENERATED_LABEL_SELECTOR,
            )
            items = result.get("items", [])
            print(
                f"Found {len(items)} {resource_type['kind']} resources to delete in namespace {namespace}"
            )
            ...
            for item in items:
                ...
                try:
                    ...
                    deleted.append(f"{resource_type['kind']}/{name}")
                    print(f"Deleted {resource_type['kind']}/{name}")
                except client.ApiException as e:
                    failed.append(f"{resource_type['kind']}/{name}: {e.reason}")
                    print(f"Failed to delete {resource_type['kind']}/{name}: {e}")
                except Exception as e:
                    failed.append(f"{resource_type['kind']}/{name}: {str(e)}")
                    print(
                        f"Unexpected error deleting {resource_type['kind']}/{name}: {e}"
                    )

        except client.ApiException as e:
            raise HTTPException(
                status_code=e.status,
                detail=f"Failed to list {resource_type['kind']}: {e.reason}",
            )
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail=f"Unexpected error listing {resource_type['kind']}: {str(e)}",
            )

    return ResetResponse(deleted=deleted, failed=failed)
